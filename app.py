...
DECISION_VERSION = os.environ.get("DECISION_VERSION", "v6.5.4.2")
...
# ─── CONFIG V6.5.4 — CONTEXTUAL BUCKET RULES / SIGNALS BETA ─────────────────
...
# ─── CONFIG V6.5.4 — HISTORICAL GUARD / WATCHLIST→TELEGRAM BETA ─────────────
# Le moteur peut annuler un CANDIDAT si la signature historique est mauvaise,
# et ne promeut automatiquement que les buckets beta explicitement activés.
ENABLE_SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA = os.environ.get(
    "ENABLE_SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA", "true"
).lower() == "true"
ENABLE_LONG_EARLY_NEUTRAL_BETA = os.environ.get(
    "ENABLE_LONG_EARLY_NEUTRAL_BETA", "false"
).lower() == "true"
ENABLE_LONG_MOMENTUM_BULLSOFT_LOW_BETA = os.environ.get(
    "ENABLE_LONG_MOMENTUM_BULLSOFT_LOW_BETA", "false"
).lower() == "true"

HISTORICAL_CACHE_FILE = os.environ.get("HISTORICAL_CACHE_FILE", "historical_performance_cache_v49.json")
HIST_PROMOTE_MIN_N = int(os.environ.get("HIST_PROMOTE_MIN_N", "30"))
HIST_PROMOTE_MIN_WR = float(os.environ.get("HIST_PROMOTE_MIN_WR", "0.68"))
HIST_BLOCK_MIN_N = int(os.environ.get("HIST_BLOCK_MIN_N", "10"))
...
TELEGRAM_ALLOWED_BUCKETS = {
    ...
    "SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA",
}
if ENABLE_LONG_EARLY_NEUTRAL_BETA:
    TELEGRAM_ALLOWED_BUCKETS.add("LONG_EARLY_NEUTRAL_BETA")
if ENABLE_LONG_MOMENTUM_BULLSOFT_LOW_BETA:
    TELEGRAM_ALLOWED_BUCKETS.add("LONG_MOMENTUM_BULLSOFT_LOW_BETA")

WATCHLIST_BUCKET_PRIORITY = {
    "LONG_EARLY_NEUTRAL_PREMIUM":             100,
    "WATCHLIST_PREMIUM_SCORE_HIGH_EARLY":      95,
    "SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA":     98,
    "LONG_EARLY_NEUTRAL_BETA":                 96,
    "LONG_MOMENTUM_BULLSOFT_LOW_BETA":         94,
    ...
    "WATCHLIST_BTC_NEUTRAL_AFTER_BULL_BLOCK":  30,
    "WATCHLIST_LONG_BLOCKED_BTC_BULL_REJECTION": 30,
    "WATCHLIST_LONG_BLOCKED_BTC_BULL_EXHAUSTION": 30,
    "WATCHLIST_LONG_BLOCKED_BTC_NEUTRAL_AFTER_BULL": 30,
    "WATCHLIST_HISTORICAL_BAD_MATCH":          25,
    "REJECT_HISTORICAL_BAD_MATCH":              7,
...
def compute_btc_market_state_details(market_details):
    ...
    # v6.5.4.2 — Rejet haussier court terme :
    ...
    if regime == "bullish" and var_4h > 0 and var_2h < -0.3 and var_30m < -0.2:
        return (
            "BTC_BULL_REJECTION",
            f"bullish 4h mais rejet CT: var2h={var_2h:+.2f}<-0.3 var30m={var_30m:+.2f}<-0.2"
        )

    # v6.5.4.2 — BTC_BULL_SOFT n'est accepté que si le court terme ne
    ...
    if (
        ...
        var_4h > BTC_BULL_SOFT_VAR4H
        and rsi > BTC_BULL_SOFT_RSI
        and var_2h > 0
    ):
        return (
            "BTC_BULL_SOFT",
            f"bull soft confirmé: regime={regime} var4h={var_4h:+.2f} var2h={var_2h:+.2f} var30m={var_30m:+.2f} rsi={rsi:.1f}"
        )
...
def compute_btc_context_v65(btc_klines_1h, market_details):
    ...
    try:
        ...
        if base_state == "BTC_BULL_REJECTION":
            bias = "transition"
            score += 0
            phase = "BTC_BULL_REJECTION"
        elif base_state in ("BTC_BULL_IMPULSE", "BTC_BULL_SOFT"):
            bias = "bullish"
            score += 2
        ...
        if phase in ("BTC_BULL_IMPULSE", "BTC_BULL_PULLBACK", "BTC_BULL_REJECTION", "BTC_BULL_EXHAUSTION", "BTC_BULL_SOFT", "BTC_NEUTRAL_AFTER_BULL"):
...
def cap_signal_count(candidates, market_regime, data_source_run):
    ...
    short_bear_exhaustion_beta_added = False
    ...
    for r in candidates:
        ...
        if bucket == "SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA":
            if short_bear_exhaustion_beta_added:
                continue
            short_bear_exhaustion_beta_added = True
        selected.append(r)
        if len(selected) >= cap:
            break
...
def load_historical_performance_cache():
    """Charge le cache historique v49. En prod Render, placer le JSON à côté de app.py."""
    candidates = [
        HISTORICAL_CACHE_FILE,
        os.path.join(os.path.dirname(os.path.abspath(__file__)), HISTORICAL_CACHE_FILE),
        "/mnt/data/historical_performance_cache_v49.json",
    ]
        ...
        try:
            if path and os.path.exists(path):
                with open(path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                if isinstance(data, dict):
                    logger.info("historical cache loaded: %s (%d keys)", path, len(data))
                    return data
        except Exception as exc:
            logger.warning("historical cache load failed %s: %s", path, exc)
    logger.warning("historical cache unavailable; guard will run with hardcoded rules only")
    return {}
...
def build_historical_key_v654(direction, entry_type, btc_phase, volume_relatif, source):
    return "|".join([
        str(direction or "").upper(),
        str(entry_type or "").upper(),
        str(btc_phase or "BTC_UNCLEAR"),
        historical_volume_band_v654(volume_relatif),
        str(source or "all"),
    ])


def find_historical_match_v654(direction, entry_type, btc_phase, volume_relatif):
    """Priorité version-aware : v653 > v65x > all."""
    for source in ("v653", "v65x", "all"):
        key = build_historical_key_v654(direction, entry_type, btc_phase, volume_relatif, source)
        row = HISTORICAL_PERFORMANCE_CACHE.get(key)
...
def apply_historical_performance_guard(
    *,
    ...
    """
    v6.5.4.2 — Historical Performance Guard.
    ...
    volume_quality = str(volume_quality or "unknown")
    try:
        fut_score = None if v6_score_futures in (None, "", "empty") else float(v6_score_futures)
    except Exception:
        fut_score = None
    try:
        part_score = int(float(participation_score))
    except Exception:
        part_score = 0
    actions = []
    ...
    # v6.5.4.2 — Blocage Telegram LONG sur phases BTC post-impulsion / rejet.
    # Important : on downgrade en WATCHLIST, pas en REJET, pour conserver la
    # mesure historique des LONG bloqués dans WATCHLIST_LOG.
    LONG_BLOCK_PHASES = {
        "BTC_BULL_REJECTION",
        "BTC_BULL_EXHAUSTION",
        "BTC_NEUTRAL_AFTER_BULL",
    }
    if flag == "CANDIDAT" and direction == "LONG" and btc_phase in LONG_BLOCK_PHASES:
        flag = "WATCHLIST"
        confidence = min(confidence, 60)
        max_leverage = min(max_leverage, 3)
        signal_quality_bucket = f"WATCHLIST_LONG_BLOCKED_{btc_phase}"
        ...
        decision_explain = f"WATCHLIST v6.5.4.2 : LONG bloqué Telegram — btc_phase={btc_phase}"
        actions.append(f"long_blocked_{btc_phase.lower()}")

    # P5 — blocage Telegram en BTC_NEUTRAL_AFTER_BULL : phase tracker v49 dangereuse.
    if flag == "CANDIDAT" and btc_phase == "BTC_NEUTRAL_AFTER_BULL":
        flag = "WATCHLIST"
        confidence = min(confidence, 60)
        max_leverage = min(max_leverage, 3)
        signal_quality_bucket = "WATCHLIST_BTC_NEUTRAL_AFTER_BULL_BLOCK"
        ...
        decision_explain = "WATCHLIST v6.5.4.2 : Telegram bloqué en BTC_NEUTRAL_AFTER_BULL, contexte historiquement faible."
        actions.append("block_btc_neutral_after_bull")

    # P4 — LONG_PREMIUM durci : volume + futures immédiats + participation.
    # v6.5.4.2 corrige le cas observé ETH : LONG_PREMIUM Telegram alors que
    ...
    if flag == "CANDIDAT" and signal_quality_bucket == "LONG_PREMIUM":
        long_blockers = []
        if vol >= 0.60:
            long_blockers.append(f"volume {vol:.2f}x>=0.60")
        if btc_phase in ("BTC_NEUTRAL_AFTER_BULL", "BTC_BULL_REJECTION", "BTC_BULL_EXHAUSTION"):
            long_blockers.append(f"btc_phase={btc_phase}")
        if market_danger_level == "HIGH":
            long_blockers.append("market_danger HIGH")
        if crowding_state in ("crowded", "extreme_crowding"):
            long_blockers.append(f"crowding={crowding_state}")
        if futures_zone == "unavailable" or fut_score is None:
            long_blockers.append("futures unavailable")
        elif futures_zone == "weak" or fut_score < 50:
            long_blockers.append(f"futures weak ({fut_score:.1f}<50)")
        if derivatives_alignment == "unavailable":
            long_blockers.append("derivatives unavailable")
        if derivatives_alignment == "opposed":
            long_blockers.append("derivatives opposed")
        if part_score <= -2:
            long_blockers.append(f"participation_score {part_score}<=-2")
        if vol < 0.50 and futures_zone in ("unavailable", "weak"):
            long_blockers.append(f"volume faible {vol:.2f}x + futures {futures_zone}")
        if volume_quality in ("weak", "danger") and futures_zone in ("unavailable", "weak"):
            long_blockers.append(f"volume_quality={volume_quality} + futures {futures_zone}")
        if long_blockers:
            flag = "WATCHLIST"
            confidence = min(confidence, 60)
            max_leverage = min(max_leverage, 3)
            signal_quality_bucket = "WATCHLIST_LONG_PREMIUM_NO_FUTURES" if (futures_zone == "unavailable" or fut_score is None) else "WATCHLIST_LONG_PREMIUM_V654_GUARD"
            ...
            decision_explain = "WATCHLIST v6.5.4.2 : LONG_PREMIUM downgradé (" + "; ".join(long_blockers) + ")."
            actions.append("downgrade_long_premium_v6541")

    # R_P1 — Promotion immédiate : SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA.
    short_bear_exhaustion_ok = (
        ENABLE_SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA
        and flag != "REJET"
        and direction == "SHORT"
        and entry_type == "MOMENTUM"
        and btc_phase == "BTC_BEAR_EXHAUSTION"
        and btc_var_30m < 0
        and 0.20 <= pr < 0.60
        and vol < 1.00
        and late < 40
    ...
    if short_bear_exhaustion_ok:
        flag = "CANDIDAT"
        signal_quality_bucket = "SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA"
        ...
        risk_guard_reason = "short momentum bear exhaustion beta"
        ...
        decision_explain = (
            f"CANDIDAT v6.5.4.2 beta : SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA "
    ...
    # R_P3 — Préparé, désactivé par défaut : LONG_EARLY_NEUTRAL_BETA.
    long_early_neutral_ok = (
        ENABLE_LONG_EARLY_NEUTRAL_BETA
        and flag != "REJET"
        and direction == "LONG"
        and entry_type == "EARLY"
        and signal_quality_bucket == "LONG_EARLY_NEUTRAL_PREMIUM"
        and global_score < 60
        and btc_phase in ("BTC_RANGE_CHOP", "BTC_NEUTRAL_ACCUMULATION", "BTC_NEUTRAL_AFTER_BEAR")
        and market_danger_level != "HIGH"
        and data_source != "SPOT_FALLBACK"
        and rr_valid
    )
    if long_early_neutral_ok:
        flag = "CANDIDAT"
        signal_quality_bucket = "LONG_EARLY_NEUTRAL_BETA"
        confidence = max(58, min(confidence, 62))
        max_leverage = min(max_leverage, 3)
        ...
        decision_explain = f"CANDIDAT v6.5.4.2 beta : LONG_EARLY_NEUTRAL_BETA en {btc_phase}, score={global_score:.1f}."
    ...
    long_bullsoft_low_ok = (
        ENABLE_LONG_MOMENTUM_BULLSOFT_LOW_BETA
        and flag != "REJET"
        and direction == "LONG"
        and entry_type == "MOMENTUM"
        and btc_phase == "BTC_BULL_SOFT"
        and historical_volume_band_v654(vol) == "low"
        and market_danger_level != "HIGH"
        and data_source != "SPOT_FALLBACK"
        and rr_valid
        and crowding_state not in ("crowded", "extreme_crowding")
    )
    if long_bullsoft_low_ok:
        flag = "CANDIDAT"
        signal_quality_bucket = "LONG_MOMENTUM_BULLSOFT_LOW_BETA"
        confidence = max(58, min(confidence, 62))
        max_leverage = min(max_leverage, 3)
        ...
        decision_explain = f"CANDIDAT v6.5.4.2 beta historique : LONG BTC_BULL_SOFT volume low. {hist_note}"
    ...
    beta_bucket = signal_quality_bucket in {
        "SHORT_MOMENTUM_BEAR_EXHAUSTION_BETA",
        "LONG_EARLY_NEUTRAL_BETA",
        "LONG_MOMENTUM_BULLSOFT_LOW_BETA",
    }
    if hist and flag == "CANDIDAT" and not beta_bucket:
        ...
        if n >= HIST_REJECT_MIN_N and wr <= HIST_REJECT_MAX_WR:
            ...
            decision_explain = f"REJET v6.5.4.2 : signature historique trop faible ({hist_note})."
            ...
            decision_explain = f"WATCHLIST v6.5.4.2 : signature historique faible ({hist_note})."
...
def apply_contextual_v652_rules(
    *,
    ...
    """
    v6.5.4.2 — Préqualification contextuelle, appelée AVANT le bucket engine.
    ...
    if short_momentum_beta_ok:
        ...
        decision = (
            "CANDIDAT v6.5.4.2 beta : SHORT_MOMENTUM_CONTINUATION_PREMIUM "
    ...
    if setup_family == "LONG_LATE_MOMENTUM" and not long_late_exception:
        ...
        decision = decision or (
            f"WATCHLIST v6.5.4.2 : LONG_LATE_MOMENTUM downgradé "
    ...
    if is_short_early:
        if btc_phase in ("BTC_BULL_SOFT", "BTC_BULL_IMPULSE", "BTC_SWITCH_RISK"):
            ...
            decision = (
                f"REJET v6.5.4.2 : SHORT_EARLY en {btc_phase}, "
            ...
            decision = decision or (
                f"WATCHLIST_DIAG v6.5.4.2 : SHORT_EARLY en {btc_phase} avec btc30m négatif. "
                "Diagnostic uniquement, pas premium en v6.5.4.2."
            )
            ...
            decision = decision or (
                f"WATCHLIST v6.5.4.2 : SHORT_EARLY non premium "
    ...
    if direction == "LONG" and trend_strength == "strong" and not long_strong_valid:
        ...
        decision = decision or (
            f"WATCHLIST v6.5.4.2 : LONG strong non premium "
    ...
    if vol > 1.20 and setup_maturity in ("late", "exhausted"):
        ...
        decision = decision or (
            f"WATCHLIST v6.5.4.2 : volume élevé ({vol:.2f}x) sur setup {setup_maturity}, "
...
def apply_contextual_bucket_engine(
    *,
    ...
    is_long_premium = (
        ...
        and relative_vol is not None and 0.30 <= relative_vol < 0.60
        and position_range is not None and position_range < pr_threshold_lp
        and btc_phase not in ("BTC_NEUTRAL_AFTER_BULL", "BTC_BULL_REJECTION", "BTC_BULL_EXHAUSTION")
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        if is_bull_soft or is_switch_risk:
            ...
            # Contexte bearish / after-bear : garder en diagnostic, sauf si le layer v6.5.4.2
            # l'a déjà préqualifié en SHORT_MOMENTUM_CONTINUATION_PREMIUM beta.
            decision_explain = (
                f"WATCHLIST_DIAG v6.5.4.2 : SHORT MOMENTUM en {btc_phase} "
            ...
            decision_explain = (
                f"WATCHLIST v6.5.4.2 : SHORT MOMENTUM non exécutable hors contexte beta "
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        if is_bull_soft or market_regime == "bearish":
            ...
            decision_explain = (
                f"WATCHLIST v6.5.4.2 : LONG trend strong en {btc_market_state} "
            ...
            telegram_rule_notes = "LONG strong: diagnostic uniquement, premium désactivé v6.5.4.2"
        else:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # V6.5.4 — Overrides contextuels après règles v6.4.4, avant gate Telegram
    ...
    # mais que le contexte v6.5.4.2 détecte un LONG MOMENTUM fragile, downgrade.
    if signal_quality_bucket == "LONG_PREMIUM" and "downgrade_bullsoft_longpremium" in v652_actions:
        ...
        decision_explain = (
            "WATCHLIST v6.5.4.2 : LONG_PREMIUM en BTC_BULL_SOFT downgradé "
            f"({'; '.join(blockers)})."
        )

    # Overrides généraux v6.5.4.2 : s'appliquent si pas de hard REJET déjà posé,
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # RÈGLE 6 — Porte Telegram stricte (v6.5.4.2)
    ...
    if flag == "CANDIDAT" and signal_quality_bucket not in TELEGRAM_ALLOWED_BUCKETS:
        ...
        decision_explain = (
            ...
            f"car bucket={signal_quality_bucket}, bucket non autorisé Telegram v6.5.4.2."
        )
    ...
    # ── v6.5.4.2 — Nettoyage final buckets premium incohérents ────────────────
    # Cas observé : un hard reject post-v6.5.4.2 gardait parfois un bucket
    ...
    if flag == "REJET" and str(signal_quality_bucket).startswith("WATCHLIST_PREMIUM_"):
        if "downgrade_long_late_momentum" in v652_actions:
            ...
            if decision_explain.startswith("WATCHLIST") or "WATCHLIST_PREMIUM" in decision_explain:
                decision_explain = "REJET v6.5.4.2 : LONG_LATE_MOMENTUM incompatible avec un bucket premium."
            ...
            if decision_explain.startswith("WATCHLIST") or "WATCHLIST_PREMIUM" in decision_explain:
                decision_explain = "REJET v6.5.4.2 : LONG strong ne remplit pas les conditions premium."
        else:
        ...
        if "WATCHLIST_PREMIUM" in decision_explain:
            decision_explain = "WATCHLIST v6.5.4.2 : LONG strong downgradé en diagnostic, conditions premium insuffisantes."
...
def score_symbol(symbol, ticker_data=None, market_regime="unknown", market_details=None):
    ...
    # v6.5.4.2 : on conserve le guard, mais on évite de tuer préventivement le
    ...
    # ── v6.5.4.2 — Instrumentation setup/participation AVANT bucket engine ───
    # v6.5.0 instrumentait ces champs après le bucket engine. En v6.5.4.2 ils
    ...
    # ── V6.5.4 — Contextual pre-qualification layer ────────────────────────
    ...
    # ── V6.4.4-final-clean + V6.5.4 context — TELEGRAM BUCKET ENGINE ────────
...
def validate_decision_config():
    ...
    expected_btc_phases = {
        ...
        "BTC_BEAR_EXHAUSTION",
        "BTC_SWITCH_RISK",
        "BTC_NEUTRAL_COMPRESS",
        "BTC_NEUTRAL_ACCUMULATION",
        "BTC_NEUTRAL_AFTER_BULL",
        "BTC_NEUTRAL_AFTER_BEAR",
        "BTC_RANGE_CHOP",
        "BTC_UNCLEAR",
    }
    if "BTC_BULL_REJECTION" not in expected_btc_phases:
        warnings.append("BTC_BULL_REJECTION absent de la liste des phases BTC")
    for bucket in (
    ...
    if ENABLE_LONG_EARLY_NEUTRAL_BETA:
        allowed_expected.add("LONG_EARLY_NEUTRAL_BETA")
    if ENABLE_LONG_MOMENTUM_BULLSOFT_LOW_BETA:
        allowed_expected.add("LONG_MOMENTUM_BULLSOFT_LOW_BETA")
    extra_telegram = TELEGRAM_ALLOWED_BUCKETS - allowed_expected
    if extra_telegram:
...
def run_v6542_regression_tests():
    """Tests de régression ciblés v6.5.4.2, sans appels réseau."""
    rejection_state, _ = compute_btc_market_state_details({
        "market_regime": "bullish",
        "btc_variation_4h": +0.8,
        "btc_variation_2h": -0.5,
        "btc_variation_30m": -0.3,
        "btc_rsi": 54,
    })
    assert rejection_state == "BTC_BULL_REJECTION", rejection_state

    bullsoft_state, _ = compute_btc_market_state_details({
        "market_regime": "bullish",
        "btc_variation_4h": +1.2,
        "btc_variation_2h": +0.3,
        "btc_variation_30m": +0.1,
        "btc_rsi": 58,
    })
    assert bullsoft_state == "BTC_BULL_SOFT", bullsoft_state


if os.environ.get("RUN_V6542_REGRESSION_TESTS", "false").lower() == "true":
    run_v6542_regression_tests()
